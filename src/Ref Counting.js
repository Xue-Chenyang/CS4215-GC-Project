...
function initialize_machine(heapsize) {
    ...
    FREE = 0;
    A = 0;
    while (A + NODE_SIZE <= HEAP_SIZE) {
        if (A + 2 * NODE_SIZE <= HEAP_SIZE) {
            HEAP[A + NEXT_SLOT] = A + NODE_SIZE;
        } else {
...
function GC() {
    display("Carring out GC!");
    if (TEMP_ROOT >= 0) {
        HEAP[TEMP_ROOT + REF_COUNT_SLOT] = HEAP[TEMP_ROOT + REF_COUNT_SLOT] + 1;
    } else {}
    HEAP[OS + REF_COUNT_SLOT] = HEAP[OS + REF_COUNT_SLOT] + 1;
    HEAP[ENV + REF_COUNT_SLOT] = HEAP[ENV + REF_COUNT_SLOT] + 1;
    LAST_FREE = -Infinity;
    FREE = HEAP_SIZE;
    QUEUE = []; QUEUE_SIZE = 0;
    for (R = 0; R <= HEAP_BOTTOM; R = R + NODE_SIZE) {
        if (HEAP[R + REF_COUNT_SLOT] === 0) {
            QUEUE[QUEUE_SIZE] = R;
    ...
    for (R = 0; R < QUEUE_SIZE; R = R + 1) {
        S = QUEUE[R];
        for (Q = HEAP[S + FIRST_CHILD_SLOT]; Q <= HEAP[S + LAST_CHILD_SLOT]; Q = Q + 1) {
            HEAP[HEAP[S + Q] + REF_COUNT_SLOT] = HEAP[HEAP[S + Q] + REF_COUNT_SLOT] - 1;
            if (HEAP[HEAP[S + Q] + REF_COUNT_SLOT] === 0) {
                QUEUE[QUEUE_SIZE] = HEAP[S + Q];
                QUEUE_SIZE = QUEUE_SIZE + 1;
            } else {}
        }
    }
...
// changes A, B, C, D, J, K
function NEW() {
    // display("Creating node at " + stringify(FREE));
    J = A;
    // K = B;
    if (FREE > HEAP_BOTTOM) {
        GC();
...
function PUSH_OS() {
    ...
    HEAP[OS + B] = A;
    HEAP[A + REF_COUNT_SLOT] = HEAP[A + REF_COUNT_SLOT] + 1;
}
...
function POP_OS() {
    ...
    RES = HEAP[OS + B];
    HEAP[HEAP[OS + B] + REF_COUNT_SLOT] = HEAP[HEAP[OS + B] + REF_COUNT_SLOT] - 1;
    HEAP[OS + B] = undefined;
}
...
function SET_CLOSURE_ENV() {
    if (HEAP[A + CLOSURE_ENV_SLOT] !== undefined) {
        HEAP[HEAP[A + CLOSURE_ENV_SLOT] + REF_COUNT_SLOT] = HEAP[HEAP[A + CLOSURE_ENV_SLOT] + REF_COUNT_SLOT] - 1;
    } else {}
    HEAP[A + CLOSURE_ENV_SLOT] = B;
    HEAP[B + REF_COUNT_SLOT] = HEAP[B + REF_COUNT_SLOT] + 1;
}
...
function POP_RTS() {
    ...
    TOP_RTS = TOP_RTS - 1;
    HEAP[RES + REF_COUNT_SLOT] = HEAP[RES + REF_COUNT_SLOT] - 1;
}
...
function EXTEND() {
    if (HEAP[A + LAST_CHILD_SLOT] + B >= NODE_SIZE) {
        ...
        for (Q = HEAP[TEMP_ROOT + FIRST_CHILD_SLOT]; Q <= HEAP[TEMP_ROOT + LAST_CHILD_SLOT]; Q = Q + 1) {
            ...
            HEAP[HEAP[RES + Q] + REF_COUNT_SLOT] = HEAP[HEAP[RES + Q] + REF_COUNT_SLOT] + 1;
        }
        TEMP_ROOT = -1;
    }   
}
